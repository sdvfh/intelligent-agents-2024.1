...
class Astar():
    def __init__(self,real_dist_path,straightline_dist_path):
        self.neighbors_dict,self.straightline_distances = DataLoader(real_dist_path,straightline_dist_path).return_dicts()
        self.frontier = {}
    ...
    def create_initial_frontier(self):
        ...
        for node in initial_frontier:
            node_cost,heuristic_cost,new_total_distance = self.heuristic_cost_estimate(0,self.start_node,node,self.goal_node)
    ...
    def calculate_A_star(self,start_node,goal_node):
        ...
        while self.frontier:
            ...
            print(f'selected node: {current_node} | f: {self.frontier[current_node]["f"]} | h: {self.frontier[current_node]["h"]} | g: {self.frontier[current_node]["g"]} | path: {self.frontier[current_node]["full_path"]}')
            if current_node == self.goal_node:
                print(f'Found goal node: {current_node} | path: {self.frontier[current_node]["full_path"]} | f: {self.frontier[current_node]["f"]} | h: {self.frontier[current_node]["h"]} | g: {self.frontier[current_node]["g"]}')
                break
            ...
            for neighbor_node in self.neighbors(current_node):
                node_cost,heuristic_cost,new_total_distance = self.heuristic_cost_estimate(current_node_cost,current_node,neighbor_node,self.goal_node)
